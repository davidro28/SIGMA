...
function Tickets_mantenimiento() {
  ...
  const ticketsFiltrados = tickets.filter(ticket => {
    ...
    const coincideBusqueda =
      ...
      ticket.activoNombre?.toLowerCase().includes(texto) ||
      ticket.responsableNombre?.toLowerCase().includes(texto) ||
      ticket.tit?.toLowerCase().includes(texto);

    const estadoLabel = getLabelEstado(ticket.est);
    const coincideEstado = estadoFiltro === "Todos" || estadoLabel === estadoFiltro;

    const prioridadLabel = getLabelPrioridad(ticket.priori);
    const coincidePrioridad = prioridadFiltro === "Todas" || prioridadLabel === prioridadFiltro;
  ...
  const handleTomarTicket = async (id) => {
    try {
      await ticketService.cambiarEstado(id, "EN_PROGRESO");
      setTickets(prev =>
        prev.map(t => t.id === id ? { ...t, est: "EN_PROGRESO" } : t)
      );
    } catch (err) {
      ...
      <div className="layout-main">
        ...
        <main className="page-content-gestor">
          <div className="header-tickets-gestor">
            ...
            <p>Gestiona los tickets asignados, actualiza estados y prioriza tu trabajo diario</p>
          </div>

          <section className="filtros-rapidos-gestor">
            <h4>Listado de tickets</h4>
            <p className="descripcion-filtros">
              Filtra por estado, prioridad o activo para encontrar rápidamente el próximo ticket a atender
            </p>
          ...
          {loading ? (
            <p style={{ padding: "1rem" }}>Cargando tickets...</p>
          ) : (
              ...
              <tbody>
                {ticketsFiltrados.length === 0 ? (
                  ...
                  ticketsFiltrados.map(ticket => {
                    ...
                    return (
                      <tr key={ticket.id}>
                        <td>{ticket.numero || ticket.id}</td>
                        <td>
                          <span className={`prioridad prioridad-${prioridadLabel.toLowerCase()}`}>
                            {prioridadLabel}
                          </span>
                        </td>
                        <td>
                          <span className={`estado estado-${estadoLabel.toLowerCase().replace(" ", "-")}`}>
                            {estadoLabel}
                          </span>
                        </td>
                        <td>{ticket.activoNombre || ticket.activoId || "—"}</td>
                        <td>{formatFecha(ticket.fechaCreacion)}</td>
                        <td>{ticket.responsableNombre || "—"}</td>
                        <td className="resumen">{ticket.tit}</td>
                        <td className="acciones">
                          {ticket.est === "ABIERTO" || ticket.est === "PENDIENTE" ? (
                            <button className="btn-tomar" onClick={() => handleTomarTicket(ticket.id)}>
                              Tomar ticket
                            </button>
                          ) : ticket.est === "EN_PROGRESO" ? (
                            <button className="btn-actualizar" onClick={() =>
                              navigate(`/DetallesTickets_mantenimiento`, { state: { ticket } })
                            }>
                              Actualizar estado
                            </button>
                          ) : null}
          ...
          <div className="nota-final">
            ...
            <p>
              Asegúrate de que el ticket esté asignado a tu equipo o utiliza los filtros para
              visualizar el histórico completo.
            </p>
          </div>
        </main>
      </div>
