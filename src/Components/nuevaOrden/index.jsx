...
function NuevaOrden({ onOrdenCreada }) {
    ...
    useEffect(() => {
        if (open) {
            ...
            fetch("http://localhost:8080/api/usuarios/por-rol?rol=Tecni_Mantenimiento", {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
            })
                .then(r => r.json())
    ...
    const handleChange = (e) => {
        ...
        // Si cambia el activo, guardar también el nombre
        if (name === "activoId") {
            ...
            setForm({
                ...
                activoInfo: activo?.serie ? `${activo.serie} · ${activo.estado}` : ""
            });
            return;
        }

        // Si cambia el técnico, guardar también el nombre
        if (name === "tecnicoId") {
            const tecnico = tecnicos.find(t => t.id === value);
            setForm({
                ...form,
                tecnicoId: value,
                tecnicoNombre: tecnico?.nombre || tecnico?.user || ""
            });
            return;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setOpen(false);
            setForm({
                activoId: "", activoNombre: "", activoInfo: "",
                tecnicoId: "", tecnicoNombre: "",
                tipo: "", estado: "PENDIENTE", prioridad: "MEDIA",
                ventana: "", ventanaSub: "", origen: "", origenId: "", descripcion: ""
            });
        } catch (err) {
            console.error("Error creando orden:", err);
            showModal("error", "Error al crear la orden", err.message || "No se pudo conectar con el servidor.", "Cerrar");
        } finally {
            setGuardando(false);
        }
    };
                        ...
                        <form className="form-orden" onSubmit={handleSubmit}>
                            ...
                            <div className="form-section">
                                ...
                                <div className="form-grid">
                                    ...
                                    <div className="form-group">
                                        <label>Ventana planificada</label>
                                        <input
                                            type="text"
                                            name="ventana"
                                    ...
                                    <div className="form-group full">
                                        <label>Subventana / retraso</label>
                                        <input
                                            type="text"
                                            name="ventanaSub"
